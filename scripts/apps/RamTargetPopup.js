...
export class RamTargetPopup extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  _onRender(context, options) {
    ...
    this.element.querySelectorAll("[data-ram-thrust]").forEach(input => {
  ...
  async _onConfirmRam(tokenId) {
    ...
    const selectedThrustPct = target.selectedThrustPct;
    const confirmed = await foundry.applications.api.DialogV2.confirm({
      ...
      content: `<p>${game.i18n.format("SHIPCOMBAT.Dialog.RamConfirmBody", {
        name:  target.name,
        pct:   Math.round(selectedThrustPct),
    ...
    const fuelUsed = this.fuelBurned + selectedThrustPct;
